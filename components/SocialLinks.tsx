const socialLinks = [
  ...
  {
    name: "LinkedIn",
    href: "https://linkedin.com/in/curtisnye",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5" aria-hidden>
    ...
    <div className={`flex items-center gap-3 ${className}`}>
      {socialLinks.map((link) => (
        <a
          ...
          className="inline-flex items-center justify-center w-11 h-11 rounded-full border border-zinc-700 text-zinc-400 hover:text-violet-400 hover:border-violet-500 transition-colors"
        >
          {link.icon}
        </a>
      ))}
